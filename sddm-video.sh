...
# Archive ZIP du repo entier — contourne LFS pour les petits fichiers binaires (PNG, etc.)
...
# /etc/sddm.conf a priorité ABSOLUE sur tout conf.d — s'il existe et contient
# [Theme], il écrasera notre zzz-sddm-video.conf à chaque reboot.
# On le renomme en .bak si c'est le cas (restaurable avec : sudo mv /etc/sddm.conf.bak /etc/sddm.conf)
if [[ -f /etc/sddm.conf ]]; then
    if grep -q '^\[Theme\]' /etc/sddm.conf 2>/dev/null; then
        warn "/etc/sddm.conf contient [Theme] — il écrase conf.d, sauvegarde en .bak"
        mv /etc/sddm.conf /etc/sddm.conf.bak
        ok "Sauvegardé : /etc/sddm.conf → /etc/sddm.conf.bak"
    else
        info "/etc/sddm.conf présent mais sans [Theme] — laissé intact"
    fi
fi
...
if command -v apt-get &>/dev/null; then
    ...
    apt-get update -qq
    apt-get install -y sddm unzip 2>/dev/null || apt-get install -y sddm 2>/dev/null || true
elif command -v pacman &>/dev/null; then
    info "Gestionnaire : pacman"
    pacman -Sy --noconfirm sddm unzip
...
# Stratégie en 3 essais :
#   1. raw.githubusercontent.com        — fichiers non-LFS
#   2. media.githubusercontent.com      — fichiers LFS (binaires lourds)
#   3. archive ZIP du repo entier       — contourne LFS pour les petits fichiers
download_asset() {
    local asset="$1"
    ...
    if [[ $success -eq 0 ]] && _is_lfs_pointer "$dest"; then
        info "$asset : pointeur LFS sur raw — essai media.githubusercontent.com..."
        rm -f "$dest"; success=1
    fi

    # Essai 2 — media.githubusercontent.com (LFS)
    if [[ $success -ne 0 ]]; then
        ...
        if [[ $success -eq 0 ]] && _is_lfs_pointer "$dest"; then
            info "$asset : toujours pointeur LFS — essai archive ZIP du repo..."
            rm -f "$dest"; success=1
        fi
    fi

    # Essai 3 — télécharger le ZIP complet du repo et en extraire le fichier
    if [[ $success -ne 0 ]]; then
        local zip_tmp="/tmp/sddm-video-repo-$$.zip"
        local zip_dir="/tmp/sddm-video-repo-$$"
        info "Téléchargement de l'archive ZIP du dépôt pour extraire $asset..."
        local zip_ok=1
        if command -v curl &>/dev/null; then
        ...
        if [[ $zip_ok -eq 0 ]] && [[ -s "$zip_tmp" ]]; then
            if command -v unzip &>/dev/null; then
                mkdir -p "$zip_dir"
                # Le zip GitHub contient un dossier "SDDM-video-main/" à la racine
                unzip -q "$zip_tmp" "SDDM-video-main/$asset" -d "$zip_dir" 2>/dev/null
                if [[ -f "$zip_dir/SDDM-video-main/$asset" ]]; then
                    mv "$zip_dir/SDDM-video-main/$asset" "$dest"
                    success=0
                fi
                rm -rf "$zip_dir"
