...
download_asset() {
    ...
    # Essai 1 : raw.githubusercontent.com (fonctionne pour les fichiers non-LFS)
    if command -v curl &>/dev/null; then
    ...
    # Vérifier que ce qu'on a reçu est bien le fichier et pas un pointeur LFS ou une page 404 HTML
    if [[ $success -eq 0 ]] && [[ -s "$dest" ]]; then
        local first_bytes
        first_bytes=$(head -c 60 "$dest" 2>/dev/null || true)
        if echo "$first_bytes" | grep -q "git-lfs"; then
            info "$asset est en LFS — tentative via media.githubusercontent.com..."
            success=1  # on va retenter via LFS
        elif echo "$first_bytes" | grep -qiE '<!DOCTYPE|<html|404: Not Found|Not Found'; then
            warn "$asset : réponse HTTP invalide depuis raw.githubusercontent.com"
            rm -f "$dest"
            success=1
        fi
    fi

    # Essai 2 : media.githubusercontent.com (Git LFS — pour les fichiers lourds)
    if [[ $success -ne 0 ]]; then
        ...
        # Vérification finale
        if [[ $success -eq 0 ]] && [[ -s "$dest" ]]; then
            if head -c 60 "$dest" 2>/dev/null | grep -qE "git-lfs|<!DOCTYPE|<html"; then
                warn "$asset : contenu invalide reçu même via LFS"
                rm -f "$dest"
                success=1
            fi
        fi
    ...
    if [[ $success -eq 0 ]] && [[ -s "$dest" ]]; then
        ...
        warn "Impossible de télécharger $asset"
        return 1
    fi
}
