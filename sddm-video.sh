#!/bin/bash
# =============================================================================
#  sddm-video.sh  v3.2  —  by PapaOursPolaire
...
#
#  CORRECTIFS v3.2 :
#    - Logique fondamentale corrigée : les assets (loginterminalc.png,
#      default.mp4, angle-down.png) sont copiés depuis le RÉPERTOIRE LOCAL
#      du repo cloné (SCRIPT_DIR). Plus de dépendance à des téléchargements
#      GitHub/LFS pour les fichiers déjà présents localement.
#    - SCRIPT_DIR détecte automatiquement le dossier du script (= racine repo).
#    - Fallback réseau uniquement si un fichier est absent localement.
#    - set -e supprimé : plus de sortie silencieuse sur erreur non fatale.
#    - Rapport de log complet généré dans /var/log/sddm-video-install.log.
#    - Vérification et neutralisation des conflits de configuration SDDM.
# =============================================================================

set -uo pipefail

# ─── Répertoire du script = racine du repo cloné ──────────────────────────────
# "sudo bash sddm-video.sh" depuis SDDM-video/ → SCRIPT_DIR = dossier SDDM-video/
# Tous les assets (loginterminalc.png, default.mp4...) sont dans ce dossier.
...
run_diagnose() {
    ...
    for f in loginterminalc.png default.mp4 angle-down.png sddm-video.sh; do
    ...
    if [[ ! -d "$THEME_DIR" ]]; then
        ...
        for fname in Main.qml theme.conf metadata.desktop loginterminalc.png angle-down.png; do
...
if [[ -n "$SDDM_ARROW" ]]; then
    ...
    install_asset "angle-down.png" || generate_fallback_png "angle-down.png"
fi
...
// SDDM Video Background  v3.2  —  Main.qml  (Qt6)  —  PapaOursPolaire
import QtQuick 2.15
...
cat > "${THEME_DIR}/Main.qml" <<'QMLEOF'
// SDDM Video Background  v3.2  —  Main.qml  (Qt5)  —  PapaOursPolaire
import QtQuick 2.15
        ...
        Column {
            ...
            Row {
                ...
                Column {
                    width: (parent.width - 8) * 0.55; spacing: 4
                    Text { text: textConstants.session; color: "#88FF88"; font.bold: true; font.pixelSize: 12 }
                    ComboBox { id: session; width: parent.width; height: 30; font.pixelSize: 14; color: "transparent"; arrowIcon: "angle-down.png"; model: sessionModel; index: sessionModel.lastIndex; KeyNavigation.backtab: password; KeyNavigation.tab: layoutBox }
                }
                Column {
                    width: (parent.width - 8) * 0.45; spacing: 4
                    Text { text: textConstants.layout; color: "#88FF88"; font.bold: true; font.pixelSize: 12 }
                    LayoutBox { id: layoutBox; width: parent.width; height: 30; font.pixelSize: 14; color: "transparent"; arrowIcon: "angle-down.png"; KeyNavigation.backtab: session; KeyNavigation.tab: loginButton }
                }
            }
            ...
            Row {
                anchors.horizontalCenter: parent.horizontalCenter; spacing: 6
                Button { id: loginButton;   text: textConstants.login;   width: 73; height: 75; color: "transparent"; textColor: "green";  onClicked: sddm.login(name.text, password.text, sessionIndex); KeyNavigation.backtab: layoutBox;  KeyNavigation.tab: shutdownButton }
...
nb_manquants=0
for check_f in Main.qml theme.conf metadata.desktop loginterminalc.png angle-down.png; do
