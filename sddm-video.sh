 ...
 * Auteur  : PapaOursPolaire — Version : 2.1 Qt6 — Licence : GPL-3.0
 * Composants : SddmComponents 2.0 uniquement (TextBox, PasswordBox,
 *              ComboBox, LayoutBox, Button, TextConstants)
 ***************************************************************************/
...
Rectangle {
    ...
    // Fallback plein écran si la vidéo échoue
    Image {
        id: fallbackImage
        anchors.fill: parent
    ...
    // ── Horloge haut-droite (Timer inliné, aucun composant externe) ───────
    Column {
    ...
    // loginterminalc.png = image de fond du panneau, SOUS les champs
    // Dimensions fixes (comme l'original) pour éviter tout binding loop
    Rectangle {
        anchors.fill: parent
        color: "transparent"
