...
# =============================================================================
#  MODE --diagnose : rapport complet de l'état SDDM, Qt, GStreamer, IBus
# =============================================================================
if [[ "${1:-}" == "--diagnose" ]]; then

    # Pas besoin de root pour lire les fichiers — on avertit juste si absent
    SEP="─────────────────────────────────────────────────────────────────"
    ...
    echo ""
    echo -e "${CYN}╔═══════════════════════════════════════════════════════╗"
    echo -e "║   SDDM Video — Diagnostic complet                    ║"
    echo -e "╚═══════════════════════════════════════════════════════╝${NC}"
    ...
    dinfo "Bash     : $BASH_VERSION"
    dinfo "Arch     : $(uname -m)"
    if [[ -n "${WAYLAND_DISPLAY:-}" ]]; then
        dinfo "Affichage : Wayland ($WAYLAND_DISPLAY)"
    elif [[ -n "${DISPLAY:-}" ]]; then
        dinfo "Affichage : X11 ($DISPLAY)"
    else
        dwarn "Affichage : aucune variable DISPLAY/WAYLAND_DISPLAY détectée"
    fi
    ...
    if command -v sddm &>/dev/null; then
        ...
        dinfo "Version  : $(sddm --version 2>/dev/null || echo 'inconnue')"
    else
    ...
    if command -v systemctl &>/dev/null; then
        local sddm_state
        sddm_state=$(systemctl is-active sddm 2>/dev/null || echo "inconnu")
        local sddm_enabled
        sddm_enabled=$(systemctl is-enabled sddm 2>/dev/null || echo "inconnu")
        [[ "$sddm_state" == "active" ]] && dok "Service actif" || dwarn "Service : $sddm_state"
        [[ "$sddm_enabled" == "enabled" ]] && dok "Service activé au démarrage" || dwarn "Démarrage auto : $sddm_enabled"
    fi

    # ── 3. Greeter Qt ────────────────────────────────────────────────────────
    dhead "3. Greeter Qt (backend d'affichage SDDM)"
    local found_greeter=0
    for p in \
    ...
    do
        if [[ -f "$p" ]]; then
            dok "Greeter trouvé : $p"
            found_greeter=1
        fi
    done
    [[ $found_greeter -eq 0 ]] && derr "Aucun greeter sddm trouvé — SDDM ne pourra pas afficher le thème"

    # ── 4. Thème installé ────────────────────────────────────────────────────
    dhead "4. Thème sddm-video"
    if [[ -d "$THEME_DIR" ]]; then
        dok "Dossier thème : $THEME_DIR"
        # Contenu
        dinfo "Fichiers :"
        ls -lh "$THEME_DIR" 2>/dev/null | tail -n +2 | while read -r line; do
            echo "       $line"
        done
        # metadata.desktop
        echo ""
        dinfo "metadata.desktop :"
        if [[ -f "$THEME_DIR/metadata.desktop" ]]; then
            cat "$THEME_DIR/metadata.desktop" | while read -r line; do echo "       $line"; done
            local plugin_name
            plugin_name=$(grep -i 'X-KDE-PluginInfo-Name' "$THEME_DIR/metadata.desktop" 2>/dev/null | cut -d= -f2-)
            if [[ -z "$plugin_name" ]]; then
                derr "X-KDE-PluginInfo-Name manquant → SDDM ignorera ce thème et utilisera le défaut"
            elif [[ "$plugin_name" != "$THEME_NAME" ]]; then
                derr "X-KDE-PluginInfo-Name='$plugin_name' ≠ dossier='$THEME_NAME' → mismatch, thème ignoré"
            else
                dok "X-KDE-PluginInfo-Name='$plugin_name' correspond au dossier"
            fi
        else
        ...
        dinfo "theme.conf :"
        if [[ -f "$THEME_DIR/theme.conf" ]]; then
            cat "$THEME_DIR/theme.conf" | while read -r line; do echo "       $line"; done
            local bg
            bg=$(grep -E '^background=' "$THEME_DIR/theme.conf" 2>/dev/null | cut -d= -f2- | tr -d '[:space:]')
            if [[ -n "$bg" ]]; then
                if [[ -f "$THEME_DIR/$bg" ]]; then
                    local size
                    size=$(du -h "$THEME_DIR/$bg" 2>/dev/null | cut -f1)
                    dok "Vidéo '$bg' présente ($size)"
                    # Vérifier que ce n'est pas un pointeur LFS
                    if head -c 50 "$THEME_DIR/$bg" 2>/dev/null | grep -q "git-lfs"; then
                        derr "ATTENTION : '$bg' est un pointeur LFS (~130 octets), pas une vraie vidéo !"
                        derr "Relancez : sudo bash sddm-video.sh --change-video"
                    fi
                else
                    derr "Vidéo '$bg' référencée dans theme.conf mais ABSENTE du dossier thème"
                fi
            else
                dwarn "Aucune entrée 'background=' dans theme.conf"
            fi
        else
            derr "theme.conf absent"
        fi
    else
        derr "Dossier thème absent : $THEME_DIR (thème non installé)"
    fi

    # ── 5. Configuration SDDM ────────────────────────────────────────────────
    dhead "5. Configuration SDDM active"
    local has_conflict=0
    if [[ -f /etc/sddm.conf ]]; then
        dwarn "/etc/sddm.conf présent — peut entrer en conflit avec $CONF_DIR/"
        dinfo "Contenu /etc/sddm.conf :"
        cat /etc/sddm.conf | while read -r line; do echo "       $line"; done
        has_conflict=1
    fi
    if [[ -d "$CONF_DIR" ]]; then
    ...
    local current_theme
    current_theme=$(grep -rh '^Current=' /etc/sddm.conf "$CONF_DIR"/*.conf 2>/dev/null | tail -1 | cut -d= -f2-)
    if [[ -z "$current_theme" ]]; then
        dwarn "Aucun thème 'Current=' défini → SDDM utilise son thème par défaut"
    elif [[ "$current_theme" == "$THEME_NAME" ]]; then
        dok "Thème actif : '$current_theme'"
    else
        dwarn "Thème actif : '$current_theme' (attendu : '$THEME_NAME')"
    fi

    # ── 6. QtMultimedia et modules QML ───────────────────────────────────────
    dhead "6. QtMultimedia — modules QML"
    local qt_mods_found=0
    for mod in \
    ...
    do
        ...
        if command -v dpkg &>/dev/null; then
            if dpkg -l "$mod" 2>/dev/null | grep -q "^ii"; then
                dok "dpkg : $mod installé"
                qt_mods_found=1
            fi
        fi
        # pacman
        if command -v pacman &>/dev/null; then
            if pacman -Q qt6-multimedia &>/dev/null 2>&1; then
                dok "pacman : qt6-multimedia installé"
                qt_mods_found=1
            fi
        fi
        # rpm
        if command -v rpm &>/dev/null; then
            if rpm -q qt6-qtmultimedia &>/dev/null 2>&1; then
                dok "rpm : qt6-qtmultimedia installé"
                qt_mods_found=1
            fi
        fi
    done
    # Recherche directe des fichiers QML dans les chemins Qt
    for qml_path in \
        ...
        /usr/share/qt6/qml/QtMultimedia
    do
        if [[ -d "$qml_path" ]]; then
            dok "Module QML Qt6 trouvé : $qml_path"
            qt_mods_found=1
        fi
    done
    for qml_path in \
        ...
        /usr/share/qt5/qml/QtMultimedia
    do
        if [[ -d "$qml_path" ]]; then
            dok "Module QML Qt5 trouvé : $qml_path"
            qt_mods_found=1
        fi
    done
    [[ $qt_mods_found -eq 0 ]] && derr "Aucun module QtMultimedia détecté → la vidéo ne s'affichera pas"

    # ── 7. GStreamer ─────────────────────────────────────────────────────────
    dhead "7. GStreamer — décodeurs vidéo"
    if command -v gst-inspect-1.0 &>/dev/null; then
        dok "gst-inspect-1.0 disponible"
        # Vérifier les décodeurs H.264 / VP8 / VP9
        ...
        dwarn "gst-inspect-1.0 introuvable — impossible de vérifier les décodeurs"
    fi

    # ── 8. IBus — origine de l'alerte Wayland ─────────────────────────────
    dhead "8. IBus — origine de l'alerte Wayland"
    dinfo "Recherche de QT_IM_MODULE et GTK_IM_MODULE dans les fichiers d'environnement..."
    echo ""
    ...
    local files_to_check=(
        ...
        "${HOME:-/root}/.config/environment.d/30-ibus.conf"
        /etc/sddm.conf
    )
    # Ajouter aussi tous les fichiers *.conf de /etc/sddm.conf.d/
    for f in "$CONF_DIR"/*.conf; do [[ -f "$f" ]] && files_to_check+=("$f"); done
    # Et /etc/environment.d/
    for f in /etc/environment.d/*.conf; do [[ -f "$f" ]] && files_to_check+=("$f"); done

    for f in "${files_to_check[@]}"; do
        [[ -f "$f" ]] || continue
        if grep -qE 'QT_IM_MODULE|GTK_IM_MODULE|XMODIFIERS.*ibus|ibus' "$f" 2>/dev/null; then
            dwarn "Trouvé dans : $f"
            grep -nE 'QT_IM_MODULE|GTK_IM_MODULE|XMODIFIERS.*ibus|ibus' "$f" 2>/dev/null \
                | while read -r match; do echo "       $match"; done
            ibus_found=1
        fi
    done
    ...
    if [[ $ibus_found -eq 0 ]]; then
        dinfo "Aucune variable IBus trouvée dans les fichiers statiques."
        dinfo "L'alerte peut venir d'un service PAM ou d'un im-config actif."
        # Vérifier im-config
        if command -v im-config &>/dev/null; then
            dwarn "im-config est installé — vérifiez sa config :"
            dinfo "  im-config -l  →  liste les méthodes configurées"
            dinfo "  im-config -n none  →  désactiver toute méthode d'entrée"
        fi
    fi
    # État actuel (env du processus courant — limité mais indicatif)
    echo ""
    dinfo "Variables d'environnement du processus courant :"
    for var in QT_IM_MODULE GTK_IM_MODULE XMODIFIERS INPUT_METHOD; do
        local val
        val="${!var:-}"
        if [[ -n "$val" ]]; then
            dwarn "$var=$val  ← définie"
        else
            dok  "$var  (non définie)"
        fi
    done
    ...
    # ── 9. Logs SDDM récents ─────────────────────────────────────────────────
    dhead "9. Logs SDDM (50 dernières lignes)"
    if command -v journalctl &>/dev/null; then
        journalctl -u sddm -b --no-pager -n 50 2>/dev/null \
            | while read -r line; do
                if echo "$line" | grep -qiE 'error|fail|crash|fatal'; then
                    echo -e "  ${RED}$line${NC}"
                elif echo "$line" | grep -qiE 'warn|ibus|im_module'; then
                    echo -e "  ${YEL}$line${NC}"
                else
                    echo "  $line"
                fi
    ...
    # ── Résumé ───────────────────────────────────────────────────────────────
    dhead "Résumé et recommandations"
    # Vérif finale X-KDE-PluginInfo-Name
    if [[ -f "$THEME_DIR/metadata.desktop" ]]; then
        local pname
        pname=$(grep -i 'X-KDE-PluginInfo-Name' "$THEME_DIR/metadata.desktop" 2>/dev/null | cut -d= -f2-)
        [[ -z "$pname" ]] && derr "Correction requise : relancez sudo bash sddm-video.sh pour régénérer metadata.desktop"
    fi
    # IBus
    if [[ $ibus_found -gt 0 ]]; then
        echo ""
        dwarn "Pour corriger l'alerte IBus Wayland, deux options :"
        dinfo "  Option A (recommandée) — désactiver via im-config :"
        echo  "    sudo im-config -n none"
        dinfo "  Option B — éditer le fichier source trouvé ci-dessus et"
        echo  "    supprimer ou commenter les lignes QT_IM_MODULE / GTK_IM_MODULE,"
        echo  "    puis relancer : sudo systemctl restart sddm"
        dinfo "  Option C — KDE System Settings → Input Devices → Virtual Keyboard"
        echo  "    → sélectionner 'IBus Wayland' → Appliquer"
    fi
    echo ""
...
# ── Correction IBus Wayland — fichier environment.d ──────────────────────────
# L'alerte "IBus should be called from the desktop session in Wayland" vient de
# QT_IM_MODULE=ibus ou GTK_IM_MODULE=ibus définis dans l'environnement système.
# Ces variables sont héritées par la session KDE Wayland où IBus ne peut pas
# s'initialiser via le protocole X11/XIM. Le bon canal sous Wayland est le
# protocole natif "input-method-v2", géré par KDE via les Paramètres système.
#
# On écrit un fichier environment.d qui désactive ces deux variables pour toutes
# les sessions PAM (y compris la session graphique démarrée par SDDM).
# Ce fichier a priorité sur /etc/environment mais peut être surchargé par
# ~/.config/environment.d/ si l'utilisateur le souhaite.
IBUS_ENV_FILE="/etc/environment.d/60-sddm-video-no-ibus-xim.conf"

info "Écriture du fichier de désactivation IBus XIM : $IBUS_ENV_FILE"
mkdir -p /etc/environment.d
cat > "$IBUS_ENV_FILE" <<'ENVEOF'
# Généré par sddm-video.sh — supprime l'alerte IBus Wayland
#
# Sous Wayland, QT_IM_MODULE et GTK_IM_MODULE ne doivent PAS être définis
# (ou doivent être vides). Le protocole Wayland input-method-v2 prend en charge
# la saisie directement. Si IBus est configuré via "IBus Wayland" dans les
# Paramètres système KDE, ces variables sont inutiles et sources de conflits.
#
# Pour supprimer ce fichier :  sudo rm /etc/environment.d/60-sddm-video-no-ibus-xim.conf
QT_IM_MODULE=
GTK_IM_MODULE=
ENVEOF
chmod 644 "$IBUS_ENV_FILE"
ok "Fichier IBus écrit : $IBUS_ENV_FILE"
